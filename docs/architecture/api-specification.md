...
## Delivered Accounting Integration API (chunk R3.1, `014-accounting-integration`)

Contract: `specs/014-accounting-integration/contracts/accounting-integration.openapi.yaml`. Delivers external
accounting system integration (QuickBooks Online in R3.1) providing automated supplier bill ingestion,
case-insensitive supplier-to-vendor link persistence, automatic purchase-to-bill matching, and 3-way reconciliation
discrepancy derivation and resolution. The integration is strictly read-only against the connected accounting
provider (FR-013) — no endpoint or background worker in this module ever writes, creates, modifies, or voids any
entity within the external accounting software.

Role-based access control is evaluated individually per endpoint:
- Connection initiation and disconnect mutations (`POST /accounting/connect`, `POST /accounting/disconnect`):
  strictly **owner-only** (`require_role(owner)`).
- Manual sync trigger (`POST /accounting/sync`) and discrepancy resolution (`POST /accounting/discrepancies/{discrepancy_id}/resolve`):
  **owner or buyer** (`require_role(owner, buyer)`).
- Connection status inspection (`GET /accounting/connection`), synced bills list (`GET /accounting/bills`),
  and reconciliation discrepancies inspection (`GET /accounting/discrepancies`): accessible to **any authenticated tenant member**.
- OAuth callback completion (`GET /accounting/connect/callback`): public redirect endpoint, **no Bearer auth**;
  secured against CSRF forgery via constant-time HMAC-SHA256 signature verification over tenant and actor claims
  embedded in the `state` parameter.

### `POST /accounting/connect`

- Owner only (`require_role(owner)`); non-owners receive `403` (`ForbiddenError`).
- Initiation check: Checks if an active or non-disconnected connection already exists for the caller's workspace.
  If a connection row with `status <> 'disconnected'` exists, refuses with `409 Conflict`
  (`details: {resource: "accounting_connection", reason: "connection_already_exists"}`).
- CSRF state token generation: Constructs an HMAC-SHA256 signed `state` token (`_generate_state()`) containing
  the caller's `tenant_id`, `membership_id`, `user_id`, `email`, `role`, current timestamp `iat`, and a 16-byte
  cryptographic `nonce`.
- Provider authorization URL: Invokes `AccountingConnector.build_authorization_url(state)` to construct the Intuit
  OAuth 2.0 consent URL requesting the `com.intuit.quickbooks.accounting` scope.
- Returns `200 OK` with `StartConnectionResponse`:

```json
{
  "authorization_url": "https://appcenter.intuit.com/connect/oauth2?client_id=...&response_type=code&scope=com.intuit.quickbooks.accounting&redirect_uri=...&state=..."
}
```

### `GET /accounting/connect/callback`

- Public endpoint called by browser redirection from the QuickBooks OAuth consent screen; **no Bearer auth**.
- Query parameters:
  - `state` (required string): signed CSRF state token generated by `POST /accounting/connect`.
  - `code` (optional string): OAuth authorization code issued by Intuit.
  - `realmId` (aliased in Python as `realm_id`, optional string): QuickBooks company/realm identifier.
  - `error` (optional string): error code returned if the user declined consent or authorization failed (e.g. `access_denied`).
- State verification: Validates signature using constant-time comparison against the application signing secret
  (`_get_signing_secret()`) and verifies token freshness (`max_age_seconds=3600`). Rehydrates caller identity
  from the token payload to establish tenant context.
- Error and decline handling: If `error` is present, `code` is omitted, `realmId` is omitted, or state validation fails,
  the endpoint creates **no database records** (guaranteeing against half-created connections per spec Acceptance Scenario 1.3).
- Token exchange and connection creation: Exchanges authorization code for OAuth access and refresh tokens
  (`connector.exchange_code_for_tokens()`), retrieves company metadata (`connector.company_info()`) to extract `display_name`,
  and inserts an `accounting_connection` row with `status = 'active'`, `provider = 'quickbooks'`, `realm_id`, and initial credentials.
- Auditing: Records `accounting.connection_created`.
- Redirect behavior: Always returns `302 Found` (`RedirectResponse`) to the web frontend at `/accounting` (specifically
  redirecting to `/accounting`, not `/accounting/connection-settings`):
  - On success: `{web_base_url}/accounting?accounting_connected=success`
  - On failure or user decline: `{web_base_url}/accounting?accounting_connected=failed&error={error_code}`

### `GET /accounting/connection`

- Requires bearer auth. Accessible to **any authenticated tenant member**.
- Retrieves the active or most recently recorded accounting connection for the caller's workspace.
- Returns `200 OK` with `AccountingConnection`.
- If no connection has ever been established for the tenant, returns `404 Not Found` (`details: {resource: "accounting_connection"}`).
  When a prior connection has been disconnected, `GET` continues to return `200 OK` with `status: "disconnected"` and populated
  `disconnected_at` rather than 404, allowing users to inspect connection status and audit history.
- Response example:
...
```json
{
  ...
  "provider": "quickbooks",
  "display_name": "Acme Catering Services LLC",
  "status": "active",
  "connected_at": "2026-09-18T09:00:00Z",
  "last_synced_at": "2026-09-18T10:30:00Z",
...
- Owner only (`require_role(owner)`); non-owners receive `403` (`ForbiddenError`).
- Preserves historical synced data: Selects the active connection using `FOR UPDATE` and updates `status = 'disconnected'`,
  `disconnected_at = now()`, and `updated_at = now()`. The connection row is **never deleted**, preserving synced bills,
  purchase-bill matches, and discrepancy audit records for historical reporting (spec Acceptance Scenario 1.2, SC-005).
- Reconnection semantics: Because the partial unique index on `accounting_connection(tenant_id)` only enforces uniqueness
  `WHERE status <> 'disconnected'`, disconnecting frees the tenant to establish a fresh connection later without merging
  prior account data.
- Returns `404 Not Found` if no active connection exists to disconnect.
- Auditing: Records `accounting.connection_disconnected`.
- Returns `200 OK` with updated `AccountingConnection` (`status: "disconnected"`).

### `POST /accounting/sync`

- Owner or buyer role (`require_role(owner, buyer)`); non-owners/buyers receive `403` (`ForbiddenError`).
- Active connection requirement: Validates that an active connection exists (`status = 'active'`). Returns `404 Not Found`
  (`details: {resource: "accounting_connection", reason: "no_active_connection"}`) if nonexistent, disconnected, or
  requiring reauthorization (`status = 'needs_reauth'`).
- Concurrency control via PostgreSQL advisory lock:
  - Enforces mutual exclusion using a dedicated session-level PostgreSQL advisory lock:
    `SELECT pg_try_advisory_lock(hashtext(%(key)s))` keyed on `str(connection_id)`.
  - If another sync execution holds the lock, immediately raises `409 Conflict`
    (`details: {resource: "accounting_connection", reason: "sync_already_running", connection_id: "..."}`).
  - Concurrency is managed via PostgreSQL advisory locks rather than an in-table `status = 'syncing'` column, providing
    automatic cleanup if the worker or API process aborts.
- Synchronous execution model:
  - There is **no background task queue** (such as Celery, ARQ, or Redis) for manual sync triggers. While returning
    `202 Accepted` with `{"status": "enqueued"}` per the OpenAPI contract, the endpoint runs `SyncService.sync()`
    **synchronously inline** within the HTTP request.
  - The synchronization pipeline executes sequentially:
    1. Reads stored credentials via `_service_role_db`.
    2. Performs OAuth token refresh against QuickBooks; on failure, marks connection `status = 'needs_reauth'`,
       records `accounting.sync_failed`, and raises `TokenRefreshFailedError` (translated to `404 Not Found`).
       Refreshed credentials are saved back via `_service_role_db`.
    3. Fetches external vendors (`connector.list_vendors()`), executes case-insensitive name matching against existing
       `supplier` rows, and upserts `synced_vendor`.
    4. Evaluates sync date window: initial sync fetches bills recorded within the last 90 days (`connected_at - 90 days`);
       subsequent syncs are unbounded (`date(1970, 1, 1)`).
    5. Fetches external bills (`connector.list_bills(since)`), copies denormalized `matched_supplier_id` from the matched vendor,
       and upserts `synced_bill`.
    6. Executes `MatchingService.match_bills()` to automatically link qualifying bills to `purchase_record` rows.
    7. Updates `last_synced_at = now()` on `accounting_connection`.
    8. Executes `ReconciliationService.recompute_discrepancies()` to derive and persist reconciliation exceptions.
- Auditing: Records `accounting.sync_started` and `accounting.sync_completed` (with payload target containing `vendors_synced`,
  `bills_synced`, `matches_created`).
...
- Requires bearer auth. Accessible to **any authenticated tenant member**.
- Returns cursor-paginated list of synced bills for the workspace, ordered newest first (`order by b.bill_date desc, b.id desc`).
- Query parameters:
  - `cursor`: optional base64-encoded integer offset string.
  - `limit`: optional integer, capped between 1 and 100 (default 50).
  - `match_status`: optional filter enum (`matched` or `unmatched`).
- Query mechanics: Joins `synced_bill` with `synced_vendor` (to provide `vendor_name`) and `LEFT JOIN` against
  `purchase_bill_match` (to compute `matched: boolean` and populate `purchase_record_id`).
- Decimal precision: `amount` is serialized strictly as a string (never float) to maintain exact monetary precision.
- Returns `200 OK` with `SyncedBillList`:
  ...
  "items": [
    {
      ...
      "matched_supplier_id": "00000000-0000-4000-8000-000000000050",
      "amount": "1250.5000",
      "currency": "EUR",
      "bill_date": "2026-09-15",
      "provider_status": "open",
      "matched": true,
      "purchase_record_id": "00000000-0000-4000-8000-000000000701"
    }
  ],
  "next_cursor": null
}
...
- Requires bearer auth. Accessible to **any authenticated tenant member**.
- Returns cursor-paginated list of reconciliation discrepancies, ordered newest first (`order by d.detected_at desc, d.id desc`).
- Query parameters:
  - `cursor`: optional base64-encoded integer offset string.
  - `limit`: optional integer, capped between 1 and 100 (default 50).
  - `status`: optional filter enum (`open` or `resolved`, default `open`).
- Denormalized detail objects: In addition to core foreign keys (`synced_bill_id`, `purchase_record_id`), the response
  enriches each discrepancy with nested read-only projection objects (`synced_bill_detail` and `purchase_record_detail`),
  avoiding extra roundtrips for the UI:
  - `synced_bill_detail` (`SyncedBillDetail | null`): `vendor_name`, `amount` (string), `currency`, `bill_date`.
  - `purchase_record_detail` (`PurchaseRecordDetail | null`): `supplier_name`, `amount` (string), `currency`, `date`
    (derived from `coalesce(ordered_at, recorded_at)`).
- Returns `200 OK` with `ReconciliationDiscrepancyList`:
  ...
  "items": [
    {
      ...
      "synced_bill_id": "00000000-0000-4000-8000-000000000601",
      "purchase_record_id": "00000000-0000-4000-8000-000000000701",
      "status": "open",
...
### `POST /accounting/discrepancies/{discrepancy_id}/resolve`

- Owner or buyer role (`require_role(owner, buyer)`); non-owners/buyers receive `403` (`ForbiddenError`).
- Path parameter: `discrepancy_id` (UUID). Returns `404 Not Found` (`details: {resource: "reconciliation_discrepancy"}`)
  if the discrepancy does not exist in the caller's tenant.
- Request body: Optional JSON payload (`ResolveDiscrepancyRequest`):
  - `note` (optional string, max length 2000): human reviewer's explanation or resolution comment.
- Conflict check: Returns `409 Conflict` (`details: {resource: "reconciliation_discrepancy", reason: "already_resolved"}`)
  if the discrepancy is already marked resolved.
- Mutation mechanics: Column-scoped update setting `status = 'resolved'`, `resolved_by = current_membership_id()`,
  `resolved_at = now()`, and `resolution_note = note`.
- Auditing: Records `accounting.discrepancy_resolved` with target containing `reconciliation_discrepancy_id`,
  `discrepancy_type`, and `note`.
- Returns `200 OK` with the resolved `ReconciliationDiscrepancy` (including populated `synced_bill_detail` and
  `purchase_record_detail`).

---

## Delivered POS & Inventory Integration API (chunk R3.2, `015-pos-inventory-integration`)

Contract: `specs/015-pos-inventory-integration/contracts/pos-integration.openapi.yaml`. Delivers a read-only
Square POS/inventory connection, daily plus manual sync of sales and stock signals, and product matching against
`workspace_product` so existing purchasing screens can display stock-on-hand and sales-velocity context. The
provider abstraction is read-only at the type level: `PosConnector` exposes OAuth, account metadata,
`list_sales_transactions(since)`, and `list_inventory_levels()` only; no create/update/delete provider method exists.

Role-based access control is evaluated individually per endpoint:
- Connection initiation and disconnect mutations (`POST /pos/connect`, `POST /pos/disconnect`):
  strictly **owner-only** (`require_role(owner)`).
- Manual sync and manual product matching (`POST /pos/sync`, `POST /pos/signals/{signal_id}/match`):
  **owner or buyer** (`require_role(owner, buyer)`), with mutation rate limits from `rate_limit_pos_sync` and
  `rate_limit_pos_match`.
- Connection status inspection (`GET /pos/connection`) and synced signal inspection (`GET /pos/signals`):
  accessible to **any authenticated tenant member**.
- OAuth callback completion (`GET /pos/connect/callback`): public redirect endpoint, **no Bearer auth**;
  secured by an HMAC-SHA256 signed `state` parameter containing tenant and actor claims.

### `POST /pos/connect`

- Owner only (`require_role(owner)`); non-owners receive `403`.
- Refuses if a non-disconnected `pos_connection` already exists for the tenant, returning `409 Conflict`
  (`details: {resource: "pos_connection", reason: "connection_already_exists"}`).
- Generates a signed CSRF `state` token containing `tenant_id`, `membership_id`, `user_id`, `email`, `role`, `iat`,
  and a nonce, then calls `PosConnector.build_authorization_url(state)`.
- Returns `200 OK` with `StartConnectionResponse`:

```json
{
  "authorization_url": "https://connect.squareup.com/oauth2/authorize?client_id=...&state=..."
}
```

### `GET /pos/connect/callback`

- Public endpoint called by browser redirection from the POS provider; **no Bearer auth**.
- Query parameters:
  - `state` (required string): signed CSRF state token generated by `POST /pos/connect`.
  - `code` (optional string): OAuth authorization code.
  - `error` (optional string): provider error code when consent is declined or authorization fails.
- If `error` is present, `code` is omitted, state validation fails, token exchange fails, insert fails, or another
  non-disconnected connection appears before insert, the service creates no connection row and returns `302 Found` to
  `/pos?pos_connected=failed&error={error_code}`.
- On success, exchanges the code for tokens, encrypts access/refresh tokens with `shared/token_crypto.py`, fetches
  `PosAccountInfo` for the display name when available (falling back to Square merchant text if metadata lookup fails),
  inserts a new `pos_connection` row with `status = 'active'`, records `pos.connection_created`, and redirects with
  `302 Found` to `/pos?pos_connected=success`.

### `GET /pos/connection`

- Requires bearer auth. Accessible to **any authenticated tenant member**.
- Retrieves the tenant's most recently created connection row, not only active rows.
- Returns `200 OK` with `PosConnection`: `id`, `provider`, `display_name`, `status`, `connected_at`,
  `last_synced_at`, and `disconnected_at`.
- Returns `404 Not Found` (`details: {resource: "pos_connection"}`) only when no connection row exists for the tenant.
  A disconnected prior connection is returned as `status: "disconnected"`.

### `POST /pos/disconnect`

- Owner only (`require_role(owner)`); non-owners receive `403`.
- Selects the latest non-disconnected connection `FOR UPDATE`, sets `status = 'disconnected'`,
  `disconnected_at = now()`, and `updated_at = now()`. The row is never deleted.
- Returns `404 Not Found` (`details: {resource: "pos_connection"}`) when there is no active or needs-reauth connection
  to disconnect.
- Records `pos.connection_disconnected`.
- Returns `200 OK` with the updated `PosConnection`.

### `POST /pos/sync`

- Owner or buyer role (`require_role(owner, buyer)`); other roles receive `403`.
- Requires the current connection to exist and have `status = 'active'`; otherwise returns `404 Not Found`
  (`details: {resource: "pos_connection", reason: "no_active_connection"}`). If the sync service later observes a
  non-active status, that is also translated to `404` with `reason: "connection_not_active"`.
- Uses `SyncService.sync()` synchronously inside the HTTP request while returning `202 Accepted` with
  `{"status": "enqueued"}` after completion.
- Concurrency is guarded by `pg_try_advisory_lock(hashtext(connection_id))`; an in-flight sync returns `409 Conflict`
  (`details: {resource: "pos_connection", reason: "sync_already_running", connection_id: "..."}`).
- Sync pipeline:
  1. Records `pos.sync_started` immediately after advisory lock acquisition.
  2. Reads OAuth tokens through the service-role token path, refreshes if the connector supports it, and marks
     `pos_connection.status = 'needs_reauth'` on Square auth failure.
  3. Fetches sales transactions since 30 days ago and current inventory levels through the read-only connector.
  4. Upserts `synced_product_signal` by `(tenant_id, external_item_id)` only, always updating `pos_connection_id` to
     the connection used for this sync.
  5. Computes `sales_velocity_per_day` and `velocity_window_days_observed`; stock and velocity remain `null` when the
     connector reports no data rather than being coerced to zero.
  6. Runs `ProductMatchingService.match_signal()` for signals that do not already have a `pos_product_match`.
  7. Updates `pos_connection.last_synced_at` and records `pos.sync_completed` with `signals_synced` and
     `matches_created`; failures record `pos.sync_failed`.
- The persisted provisional-window field `velocity_window_days_observed` supports FR-013: a partial history window can
  be disclosed differently from a full 30-day figure. The current `SyncedProductSignal` FastAPI response model does
  **not** expose this field even though frontend templates branch on it when present; see the open question in T038's
  report.

### `GET /pos/signals`

- Requires bearer auth. Accessible to **any authenticated tenant member**.
- Returns cursor-paginated signals ordered by `synced_product_signal.created_at desc, id desc`.
- Query parameters:
  - `cursor`: optional base64-encoded integer offset string.
  - `limit`: optional integer, capped between 1 and 100 (default 50).
  - `match_status`: optional filter enum (`matched` or `unmatched`).
  - `workspace_product_id`: optional UUID filter, matching through `pos_product_match.workspace_product_id`.
- Response fields are exactly the current `SyncedProductSignal` schema: `id`, `external_item_name`, `matched`,
  `matched_workspace_product_id`, `stock_on_hand` (decimal string or null), `stock_synced_at`,
  `sales_velocity_per_day` (decimal string or null), `velocity_window_days`, and `velocity_computed_at`.

### `POST /pos/signals/{signal_id}/match`

- Owner or buyer role (`require_role(owner, buyer)`); other roles receive `403`.
- Path parameter: `signal_id` (UUID).
- Request body: `ManualMatchRequest` with required `workspace_product_id` (UUID).
- Returns `404 Not Found` if the signal or workspace product does not exist in the caller's tenant.
- Returns `409 Conflict` (`details: {resource: "pos_product_match", reason: "already_matched"}`) if the signal or the
  target product already has a match.
- Inserts a manual `pos_product_match` with `match_method = 'manual'`, `matched_by = current membership`, and
