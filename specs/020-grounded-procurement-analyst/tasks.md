...
- [ ] T007 [P] Write failing unit tests for each FR-002 retrieval category (spend/savings,
      ...
      grounding data (must say so explicitly), conflicting records (must surface both), and
      explicit-currency handling (no cross-currency aggregation)
- [ ] T008 Implement one pure retrieval-and-calculation function per category in
      ...
      `calculation_version = "analyst-retrieval-v1"` on every answer (depends on T006, T007)
...
- [ ] T011 [US1] Implement question understanding in
      ...
      it must never pass free text through into the answer itself
- [ ] T012 [US1] Implement the turn service in
      `apps/api/src/procurepilot_api/modules/analyst/service.py`: classify (T011) → retrieve
      (T008) → persist the immutable turn + citations atomically → append an audit event
      (FR-012) (depends on T008, T011)
- [ ] T013 [US1] Implement the ask-question endpoint in
      ...
      (depends on T012)
- [ ] T014 [US1] Add per-member rate limiting on the ask-question endpoint (FR-015)
- [ ] T015 [P] [US1] Write a failing component test for asking a question and seeing a cited,
...
- [ ] T016 [US1] Implement the API client and standalone conversation component in
      `apps/web/src/app/features/analyst/conversation/conversation.component.ts`/`.html`/`.scss`
- [ ] T017 [US1] Add the route to `apps/web/src/app/app.routes.ts`, the nav entry, and i18n keys
...
**Goal**: Every citation resolves to the real underlying record, and every calculation's inputs
and formula are visible, not summarized.
...
the retrieval function's actual inputs.

### Tests for User Story 2

- [ ] T018 [P] [US2] Integration test: each citation kind (purchase order, quotation line, landed
...
- [ ] T019 [P] [US2] Component test: calculation detail expands and citation links navigate to the
      correct existing screens, in `conversation.component.spec.ts`

### Implementation for User Story 2

- [ ] T020 [US2] Ensure the citation payload from `retrieval.py`/`service.py` carries stable IDs/
      links to the exact underlying record for each source kind (extends T008/T012)
- [ ] T021 [US2] Implement the citation and "show calculation" UI in
      `conversation.component.html`/`.ts` — inline expansion, links out to existing record screens

**Checkpoint**: US1 + US2 together deliver the full "ask and verify" MVP.
...
- [ ] T022 [P] [US3] Unit test: a follow-up resolves omitted entities from the immediately
      preceding turn only, not the full conversation, in `apps/api/tests/unit/test_analyst_intent.py`
- [ ] T023 [P] [US3] Integration test: a two-turn conversation where turn 2 depends on turn 1, and
      a second case where turn 2 changes subject entirely, in `test_analyst_api.py`

### Implementation for User Story 3

- [ ] T024 [US3] Extend `intent.py`/`service.py` to pass the preceding turn's resolved category
      and entities into classification for a follow-up (FR-008)
- [ ] T025 [US3] Update the conversation UI to keep the thread visible and send the prior turn's
...
- [ ] T026 [P] [US4] Contract test for `GET /api/v1/analyst/conversations` and
      `GET /api/v1/analyst/conversations/{id}` in `test_analyst_contract.py`
- [ ] T027 [P] [US4] Integration test: a member lists only their own conversations; an owner/buyer
...
- [ ] T028 [US4] Implement conversation list/detail endpoints in `router.py`/`service.py` with
      cursor pagination and the creator-scoped-plus-owner/buyer-oversight read rule (FR-009)
- [ ] T029 [US4] Implement the history UI: list past conversations, reopen one, and render its
...
- [ ] T030 [P] Add API and data-dictionary contracts for the conversation, turn, and citation
      entities in `docs/architecture/api-specification.md` and `docs/architecture/data-dictionary.md`
- [ ] T031 [P] Record ADR-017 in `docs/architecture/adrs.md`: question understanding reuses the
      existing Bedrock/Claude provider (ADR-004/ADR-014), no new LLM provider is introduced
- [ ] T032 Run API unit, integration, contract, web unit, and production build gates
- [ ] T033 Run the G3 evidence command, confirm output remains unmet, and record the R4.2
      exception posture in the release evidence doc (same pattern as R4.0/R4.1)
- [ ] T034 Run a live hosted walkthrough — one question per supported category, in English and
      Arabic — mirroring the R4.0/R4.1 verification pattern, and record it in the release
      evidence doc
...
- **User Stories (Phase 3-6)**: All depend on Foundational completion.
  - US1 and US2 are both P1 and tightly coupled (an answer without visible citations isn't
    grounded); implement them back-to-back as the MVP slice.
...
- **US2 (P1)**: Extends US1's answer payload with citation/calculation detail; not independently
  meaningful without US1, but its own tasks touch different files and can be built as soon as
  US1's retrieval/service layer (T008/T012) exists.
...
4. Complete Phase 4: US2 (inspect the evidence) — without this, US1's answers aren't actually
   verifiable, so treat US1+US2 together as the real MVP, not US1 alone.
5. **STOP and VALIDATE**: live hosted walkthrough of the MVP slice before adding US3/US4.

### Incremental Delivery

1. Setup + Foundational → foundation ready.
2. US1 + US2 → MVP: ask and verify a grounded answer.
3. US3 → conversational follow-ups.
4. US4 → question history and oversight.
5. Polish → docs, full gates, G3 evidence, live walkthrough.
