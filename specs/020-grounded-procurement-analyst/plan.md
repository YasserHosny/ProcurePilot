...
stores the question, resolved category+entities, the deterministic answer, and its citations as
immutable rows. A focused Angular chat-style feature presents the conversation with inline
citations and calculations, without any path to autonomous purchasing.
...
- Modify: `docs/quality/r4.1-release-evidence.md` (or a new `docs/quality/r4.2-release-evidence.md`
  — decide during implementation which reads better; either way, record the same G3-unmet
  exception pattern used for R4.0/R4.1)
...
- [ ] **Step 1: Write failing tests** for each FR-002 category (spend/savings, supplier
  ...
  conflicting-records case (must surface both, not pick one), and explicit currency handling
  (no cross-currency aggregation, matching R4.0/R4.1's `Decimal` convention).
...
- [ ] **Step 2: Implement question understanding** — a thin call to the existing Bedrock/Claude
  ...
  (FR-008), never the full conversation.
- [ ] **Step 3: Implement the turn service** — classify, retrieve (Task 2), persist the immutable
  turn + citations atomically, append an audit event (FR-012), and expose
  `release_posture = g3_unmet` on every response.
...
- [ ] **Step 1: Write failing component tests** for asking a question, an answer with inline
  citations and a visible calculation, the "can't answer that yet" state (FR-002), the "no
  grounding data" state (FR-006), and a follow-up turn.
- [ ] **Step 2: Implement the API client and standalone component** — a chat-style thread with
  ...
  underlying record screens used elsewhere in the product (FR-003), not a summary or copy.
