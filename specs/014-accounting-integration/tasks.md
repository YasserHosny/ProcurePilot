...
- [x] **T018** [US2] — `SyncService.sync(connection)`: calls the connector's `list_vendors()`
...
- [x] **T019** [US2] — `MatchingService.match_bill(bill)`: finds `purchase_record` candidates for
...
- [x] **T020** [P] [US2] — Unit tests for `MatchingService`'s matching/tie-breaking logic
...
- [x] **T021** [US2] — Worker: `apps/api/src/procurepilot_api/workers/accounting_sync_worker.py` — daily
...
- [x] **T022** [US2] — Router: `POST /accounting/sync` (manual trigger, refuses with 409 if a
...
- [x] **T024** [P] [US2] — Integration tests in
...
- [x] **T025** [P] [US2] — Integration test for the worker's own claim loop in
...
- [x] **T033** — i18n: add the `accounting.*` namespace (connection settings, bills, discrepancy
...
- [x] **T034** — Routing + navigation: add the four `/accounting/*` routes to
...
- [x] **T035** — Tenant isolation: extend the canonical `apps/api/tests/integration/
...
- [x] **T036** — E2E: `apps/web/tests/e2e/accounting.spec.ts` — connect (stub-backed test
...
- [x] **T037** — a11y: `apps/web/tests/e2e/accounting-a11y.spec.ts` — axe-core WCAG 2.1 AA on all
  three new screens in English and Arabic, keyboard navigation for the resolve-discrepancy action.
- [x] **T038** — Docs: add the accounting integration section to
...
- [x] **T039** — Security review: OAuth token storage/encryption, the callback endpoint's `state`
