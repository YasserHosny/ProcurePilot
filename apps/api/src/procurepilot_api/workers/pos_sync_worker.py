"""Daily worker for POS and inventory synchronization (R3.2, task T019).

Polls pos_connection for rows due for sync (status = 'active' and last_synced_at is
...
def tick(settings: Settings, sync_service: SyncService | None = None) -> dict[str, int]:
    """Execute a single polling tick across all due POS connections.
    ...
    for candidate in candidates:
        ...
        try:
            ...
            logger.info(
                "Sync already running for POS connection %s; skipping",
                connection_id,
            )
            stats["skipped_conflict"] += 1
        except Exception:
            logger.exception(
                "POS sync failed for connection %s",
                connection_id,
            )
            stats["failed"] += 1
...
def _find_due_connections(settings: Settings) -> list[dict[str, object]]:
    """Select POS connections due for daily sync across all tenants.
...
def run_loop(
    ...
    *,
    ...
    """Continuous polling loop for POS synchronization.
    ...
    """
    logger.info(
        "Starting POS sync worker (once=%s, interval=%ss)",
        once,
        interval_seconds,
    )
    while True:
        try:
            stats = tick(settings)
            if stats["checked"]:
                logger.info("POS sync pass completed: %s", stats)
        except Exception:
            logger.exception("Error during POS sync worker pass")
        if once:
            break
...
def main() -> None:
    parser = argparse.ArgumentParser(description="POS synchronization worker")
